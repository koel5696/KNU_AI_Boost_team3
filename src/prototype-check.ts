...
const attachmentSources: ProcessedSource[] = [
  {
    id: "pdf-1",
    fileName: "모집요강.pdf",
    kind: "pdf",
    page: 2,
    text: "모집 대상: 19~34세 청년\n신청 마감: 2026년 8월 20일까지\n문의: help@example.com",
    size: 1200,
    isOcr: false,
...
const multiSource = analyzeSources(sampleMail, attachmentSources);
const periodField = multiSource.fields.find((field) => field.key === "period");
assert(Boolean(periodField?.sourceName), "다중 문서 결과의 출처 연결 실패");
assert(Boolean(periodField?.hasConflict), "본문과 첨부파일의 기간 충돌 감지 실패");
assert(multiSource.links.includes("https://example.com/apply"), "첨부파일 링크 수집 실패");

console.log("prototype-check: 추출, 채널 초안, 다중 문서 출처·충돌 확인 통과");
